...
type AnswerTarget = 'fixed' | 'players' | 'casts';

const isPlayerPlaceholder = (o: string): boolean => /^[A-Z]さん$/.test(o) || /^プレイヤー[A-Z]$/.test(o);
const isCastPlaceholder = (o: string): boolean => /^キャスト[A-Z]$/.test(o);

/**
 * §移行 のヒューリスティック分類。
 * 優先順位: casts（scene==='キャスト指名' or options に キャストX を含む）→ players（options 全てがプレースホルダ）→ fixed。
 * casts を players より先に判定するのは、キャスト指名プリセットの options が
 * プレイヤープレースホルダ表記でも「別集合(キャスト)」として扱うため（設計 §核心）。
 */
...
function main() {
  ...
  const now = Date.now();
  const stats = { players: 0, casts: 0, fixed: 0, correctIndexRemoved: 0 };
  ...
  for (const p of presets) {
    ...
    const questions = JSON.stringify(p.questions.map((q, i) => {
      const answerTarget = classifyAnswerTarget(q, p.scene);
      // players/casts は常に非採点。correctIndex を除去（不一致①の恒久解消 + 二重防御）。
      const isFixed = answerTarget === 'fixed';
      const hadCorrectIndex = q.correctIndex !== null && q.correctIndex !== undefined;
      if (isFixed) stats.fixed++;
      else if (answerTarget === 'players') stats.players++;
      else stats.casts++;
      if (!isFixed && hadCorrectIndex) stats.correctIndexRemoved++;

      const base: Record<string, unknown> = {
        id: randomUUID(),
        text: q.text,
        options: q.options,
        correctIndex: isFixed ? q.correctIndex : undefined,
        timeLimitSec: q.timeLimitSec,
        orderIndex: i,
      };
      // fixed は undefined='fixed' 扱いのため付与しない（ファイル肥大回避・設計許容）。
      if (!isFixed) base.answerTarget = answerTarget;
      return base;
    }));
    lines.push(
  ...
  console.log('answerTarget classification:', JSON.stringify(stats));
}
main();
